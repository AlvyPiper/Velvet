...
void init::dosigscan()
{
	...
	clientmode = **(clientmodeshared***) (util::findpattern(clienttable[8], 0x100, "\x8B\x0D????\x8B") + 0x02); //http://www.unknowncheats.me/forum/source-engine/159253-finding-pointer-clientmode-any-game.html
...
{
	client = createinterface<cclient*>("client.dll", "VClient015"); //pointer to client
...
void init::dohook()
{
	...
	clientModeHook->HookMethod(&hookmngr::createmove, 21); //replaces the pointer to CreateMove with our own
